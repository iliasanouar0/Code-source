    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            for (let i = 0; i < check.length; i++) {
                ...
                fetch(`http://${ip}:3000/cloud/lists/${ID}`, {
                    method: "DELETE",
                    headers: {
                        'Content-Type': 'application/json',
                        'Access-Control-Allow-Origin': '*',
...
$(document).on('click', '.save', event => {
    ...
    let settings = {
        "url": `http://${ip}:3000/cloud/lists/${id}?name=${name}`,
        "method": "PUT",
        "headers": {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
...
$(document).on('click', '#l_seeds_add', event => {
    ...
    let obj = []
    let settings = {
        "url": `http://${ip}:3000/cloud/lists/isp/${listId}`,
        "method": "GET",
        "headers": {
            'Content-Type': 'application/json',
            'Access-Control-Allow-Origin': '*',
    ...
    $.ajax(settings).done(function (responseText) {
        ...
        for (let i = 0; i < dataArray.length; i++) {
            console.log(dataArray[i].indexOf(',') > -1);
            if (/*dataArray[i] != '' ||*/ dataArray[i].indexOf(',') > -1) {
...
document.getElementById("demo").onchange = evt => {
    ...
    reader.addEventListener("loadend", evt => {
        ...
        $(document).on('click', '.save-bulk-data', event => {
            ...
            let obj = []
            let settings = {
                "url": `http://${ip}:3000/cloud/lists/isp/${data}`,
                "method": "GET",
                "headers": {
                    'Content-Type': 'application/json',
                    'Access-Control-Allow-Origin': '*',
