        ...
        fetch(`http://${ip}:3000/settings/columns/${elm.tablename}`, { method: "GET" })
          .then(response => {
            ...
            data.forEach(elm => {
              ...
              rows += `<tr><td>${elm.column_name}</td><td>${elm.data_type}</td><td class="text-danger column-drop" data-name="${elm.column_name}"><span class="btn btn-outline-danger">DROP</span></td></tr>`
            })
