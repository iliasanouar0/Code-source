...
const wssUri = `ws://${ip}:7075/wss?id=${user.id_user}`;
...
$(document).on("click", "#add_compose", () => {
    ...
    $('.composeEditTitle').html('Create new process')
    $('#limit_send').prop("disabled", false);
...
$(document).on('click', '.edit', event => {
    ...
    $('.composeEditTitle').html('Update process')
    fetch(`http://${ip}:3000/cloud/process/one/${id}`, {
        method: "GET",
    }).then(response => {
        ...
        if (json[0].offer == '' || json[0].offer == 'none') {
            ...
            var settings = {
                "url": `http://${ip}:3000/cloud/process/offerdata?offer=${json[0].offer}`,
                "method": "GET",
                "timeout": 0,
            };
...
const updateCompose = data => {
    var settings = {
        "url": `http://${ip}:3000/cloud/process/`,
        "method": "PUT",
        "timeout": 0,
        "headers": {
...
const handleImageUpload = event => {
    ...
    fetch(`http://${ip}:3000/cloud/process/offers/upload/`, {
        method: 'POST',
        body: formData
        ...
        offersAdd.innerHTML = ""
        fetch(`http://${ip}:3000/cloud/process/offers`, {
            method: "GET",
        }).then((response) => {
            return response.json();
        }).then((data) => {
...
$('#p_offers_add').change(event => {
    ...
    if (value == '') {
        ...
        var settings = {
            "url": `http://${ip}:3000/cloud/process/offerdata?offer=${value}`,
            "method": "GET",
            "timeout": 0,
        };
        $.ajax(settings).done(function (response) {
...
function msToMnSc(duration) {
    ...
    hours = (hours < 10 && hours > 0) ? "0" + hours : hours = (hours < 0) ? "00" : hours;
    minutes = (minutes < 10 && minutes > 0) ? "0" + minutes : minutes = (minutes < 0) ? "00" : minutes;
    seconds = (seconds < 10 && seconds > 0) ? "0" + seconds : seconds = (seconds < 0) ? "00" : seconds;
...
$(document).on('click', '.status', event => {
    ...
    $('#process_result').DataTable({
        ...
        autoWidth: false,
        ajax: {
            url: `http://${ip}:3000/cloud/process/seeds/${id}`,
            dataSrc: '',
        },
...
const editActions = (data) => {
    console.log(data);
    var settings = {
        "url": `http://${ip}:3000/cloud/process/actions/${data.id_process}`,
        "method": "POST",
        "timeout": 0,
        "headers": {
            "Content-Type": "application/json"
...
$(document).on('click', '.delete-all-this', () => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            fetch(`http://${ip}:3000/cloud/process/`, {
                method: "PATCH",
...
const data = $('#edit_offers').DataTable({
    ...
    autoWidth: false,
    ajax: {
        url: `http://${ip}:3000/cloud/process/offers`,
        dataSrc: '',
    },
    columns: [
        {
...
$(document).on('click', '.delete_offer', event => {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            console.log(offer);
            fetch(`http://${ip}:3000/cloud/process/offer/${offer}`, {
                method: "DELETE",
            }).then(res => {
                return res.text()
            }).then(() => {
