...
const getListsSup = (req, res) => {
  pool.query(
    "SELECT list.*, COUNT(id_seeds) AS seeds_count, users.login,users.type, entity.nom FROM list LEFT JOIN seeds ON seeds.id_list = list.id_list JOIN users ON list.id_user=users.id_user JOIN entity ON users.id_entity=entity.id_entity WHERE users.type!='IT' GROUP BY 1,users.id_user,users.id_entity,entity.id_entity ORDER BY list.date_add DESC",
    (error, results) => {
      if (error) {
