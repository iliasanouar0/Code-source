...
const getLists = (request, response) => {
  pool.query(
    "SELECT list.*, COUNT(id_seeds) AS seeds_count, users.*, entity.nom FROM list LEFT JOIN seeds ON seeds.id_list = list.id_list JOIN users ON list.id_user=users.id_user JOIN entity ON users.id_entity=entity.id_entity GROUP BY 1,users.id_user,users.id_entity,entity.id_entity",
    (error, results) => {
      if (error) {
