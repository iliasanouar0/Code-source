...
const getLists = (request, response) => {
  pool.query(
    "SELECT list.*, COUNT(id_seeds) AS seeds_count, users.* FROM list LEFT JOIN seeds ON seeds.id_list = list.id_list JOIN users ON list.id_user=users.id_user GROUP BY 1,users.id_user",
    (error, results) => {
      if (error) {
