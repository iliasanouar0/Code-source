...
const getAllProcessSeedsByState = async (data) => {
    ...
    // let sql = "SELECT process.id_process,process.action, results.status as rs,seeds.* FROM process JOIN results ON results.id_process=process.id_process JOIN seeds ON seeds.id_list=process.id_list WHERE process.id_process=($1) AND results.status=($2) GROUP BY seeds.id_list,process.id_list,process.id_process,seeds.id_seeds,results.status"
    let sql = "SELECT process.id_process,process.action, results.status as rs FROM process JOIN results ON results.id_process=process.id_process WHERE process.id_process=($1) AND results.status=($2) GROUP BY seeds.id_list,process.id_list,process.id_process,seeds.id_seeds,results.status"
    const client = await pool.connect()
