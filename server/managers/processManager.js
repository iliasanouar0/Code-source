...
const getAllProcessSeedsByState = async (data) => {
    ...
    let sql = "SELECT process.id_process,process.action, results.status as rs FROM process JOIN results ON results.id_process=process.id_process WHERE process.id_process=($1) AND results.status=($2) GROUP BY process.id_process,results.status"
    const client = await pool.connect()
