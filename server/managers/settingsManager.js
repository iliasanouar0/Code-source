...
const getTables = (request, response) => {
    ...
    let sql = `SELECT * FROM (SELECT tablename,COUNT(tablename),schemaname FROM pg_catalog.pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema' GROUP BY pg_tables.tablename UNION ALL SELECT  table_name,  column_name,  data_type  FROM  information_schema.columns) U WHERE U.tablename=U.table_name GROUP BY pg_tables.tablename`
