...
const getTables = (request, response) => {
    ...
    let sql = `SELECT * FROM (SELECT tablename,tableowner,schemaname FROM pg_catalog.pg_tables WHERE schemaname != 'pg_catalog' AND schemaname != 'information_schema' GROUP BY pg_tables.tablename,pg_tables.schemaname,pg_tables.tableowner UNION ALL SELECT  table_name,  column_name,  data_type  FROM  information_schema.columns) AS U WHERE U.tablename=U.table_name GROUP BY pg_tables.tablename`
