...
const createTables = (request, response) => {
    ...
    CREATE TABLE IF NOT EXISTS seeds ( Id_seeds SERIAL NOT NULL, gmail VARCHAR(50) UNIQUE, password VARCHAR(50), isp VARCHAR(50), proxy VARCHAR(50), status VARCHAR(50), date_add VARCHAR(100), date_update VARCHAR(100), verification VARCHAR(50), Id_list INT NOT NULL, PRIMARY KEY (Id_seeds), FOREIGN KEY (Id_list) REFERENCES list (Id_list) ON DELETE CASCADE);

    CREATE TABLE IF NOT EXISTS process ( Id_process SERIAL NOT NULL, name VARCHAR(50), action VARCHAR(50), status VARCHAR(50), date_add VARCHAR(100), date_update VARCHAR(100), start_in VARCHAR(100), end_in VARCHAR(100), Id_list INT NOT NULL, Id_user INT NOT NULL, PRIMARY KEY (Id_process), FOREIGN KEY (Id_list) REFERENCES list (Id_list) ON DELETE CASCADE, FOREIGN KEY (Id_user) REFERENCES users (Id_user) ON DELETE CASCADE);

    CREATE TABLE IF NOT EXISTS results (Id_list INT, Id_seeds INT, Id_process INT, id_result SERIAL NOT NULL, feedback VARCHAR(50), start_in VARCHAR(100), end_in VARCHAR(100), PRIMARY KEY (id_result), FOREIGN KEY (Id_list) REFERENCES list (Id_list) ON DELETE CASCADE, FOREIGN KEY (Id_seeds) REFERENCES seeds (Id_seeds) ON DELETE CASCADE, FOREIGN KEY (Id_process) REFERENCES process (Id_process) ON DELETE CASCADE);
