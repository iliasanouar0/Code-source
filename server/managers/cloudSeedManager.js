...
const updateSeed = (request, response) => {
  let obj = request.body;
  const sql =
    "UPDATE cloudseed SET password=($2), proxy=($3),verification=($4), update_date=(now())  WHERE id_seed=$1";
...
const updateSeeds = (request, response) => {
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    qr += `update_date=(now()) WHERE gmail=($1)`
...
const updateProxy = (request, response) => {
  ...
  const Update = "UPDATE cloudseed SET proxy=($1), update_date=(now()) WHERE proxy=($2) AND id_list=($3)";
  query.forEach((data) => {
